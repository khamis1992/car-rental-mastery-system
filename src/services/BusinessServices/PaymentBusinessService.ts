  ...
  async createPayment(paymentData: PaymentFormData): Promise<Payment> {
    try {
      ...
      // Create revenue recognition entry for the payment (revenue is recorded when payment is received)
        ...
        console.error(`❌ Failed to create revenue recognition entry for payment ${payment.id}:`, accountingError);
        // Don't fail the entire payment creation if accounting fails - log error for later reconciliation
        console.warn(`⚠️ Payment for invoice ${paymentData.invoice_id} created but revenue recognition failed. This payment can be reprocessed using the System Integrity tools.`);
      }
