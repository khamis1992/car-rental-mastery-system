-- Fix ambiguous column references in validate_journal_entry_balance function
CREATE OR REPLACE FUNCTION public.validate_journal_entry_balance()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
  entry_total_debit NUMERIC := 0;
  entry_total_credit NUMERIC := 0;
BEGIN
  SELECT 
...
-- Fix potential issues in create_invoice_accounting_entry function
...
BEGIN
  ...
  SELECT i.journal_entry_id INTO journal_entry_id FROM public.invoices i WHERE i.id = create_invoice_accounting_entry.invoice_id;
...
-- Fix potential issues in create_payment_accounting_entry function
...
BEGIN
  -- التحقق من عدم وجود قيد محاسبي مسبق
  SELECT p.journal_entry_id INTO journal_entry_id FROM public.payments p WHERE p.id = create_payment_accounting_entry.payment_id;
  IF journal_entry_id IS NOT NULL THEN
    RETURN journal_entry_id; -- القيد موجود بالفعل
  END IF;
...
-- Create function to reprocess existing invoices without accounting entries
