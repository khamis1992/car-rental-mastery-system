...
-- إضافة عمود لتمييز نوع المستأجر (platform للمنصة أو customer للعملاء)
ALTER TABLE public.tenants ADD COLUMN IF NOT EXISTS tenant_type TEXT DEFAULT 'customer' CHECK (tenant_type IN ('platform', 'customer'));
...
-- تحديث المستأجر الأول ليكون منصة (platform)
UPDATE public.tenants SET tenant_type = 'platform' WHERE id = (SELECT id FROM public.tenants ORDER BY created_at LIMIT 1);