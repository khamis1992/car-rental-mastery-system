-- تحديث دالة إيراد الدفعة لتسجيل الإيراد الفعلي
...
BEGIN
    ...
    -- تسجيل الإيراد عند الدفع (دائن)
    INSERT INTO public.journal_entry_lines (
        journal_entry_id, account_id, description, debit_amount, credit_amount, line_number
    ) VALUES (
        journal_entry_id, revenue_account, 'إيراد تأجير - ' || invoice_number || ' - ' || customer_name, 0, payment_amount, 3
    );
    
    -- تعديل المجاميع في القيد لتشمل الإيراد
    UPDATE public.journal_entries 
    SET 
        total_debit = payment_amount,
        total_credit = payment_amount * 2
