-- تصحيح دوال المحاسبة لتطبيق مبدأ الإيرادات المؤجلة

-- تحديث دالة مديونية الفاتورة لتستخدم الإيرادات المؤجلة
...
BEGIN
    ...
    -- الإيرادات المؤجلة (دائن) - الجزء غير الضريبي
    INSERT INTO public.journal_entry_lines (
        journal_entry_id, account_id, description, debit_amount, credit_amount, line_number
    ) VALUES (
        journal_entry_id, deferred_revenue_account, 'إيراد مؤجل - فاتورة ' || invoice_number, 0, total_amount - tax_amount, 2
    );
    ...
    IF tax_amount > 0 AND tax_payable_account IS NOT NULL THEN
        INSERT INTO public.journal_entry_lines (
            ...
            journal_entry_id, tax_payable_account, 'ضرائب مستحقة - فاتورة ' || invoice_number, 0, tax_amount, 3
        );
    END IF;
...
-- تحديث دالة إيراد الدفعة لتحويل الإيرادات المؤجلة إلى إيرادات محققة
...
BEGIN
    ...
    -- إنشاء القيد المحاسبي (تحصيل + تحقيق الإيراد)
    INSERT INTO public.journal_entries (
        entry_number,
        entry_date,
        description,
        ...
        journal_entry_number,
        payment_date::DATE,
        'تحصيل وتحقيق إيراد - ' || invoice_number || ' - ' || customer_name,
        'payment',
        payment_id,
        payment_amount * 2,
        payment_amount * 2,
    ...
    -- 2. الإيرادات المؤجلة (مدين) - تحويل إلى إيراد محقق
    INSERT INTO public.journal_entry_lines (
        journal_entry_id, account_id, description, debit_amount, credit_amount, line_number
    ) VALUES (
        journal_entry_id, deferred_revenue_account, 'تحويل إيراد مؤجل - ' || invoice_number, payment_amount, 0, 2
    );
    
    -- 3. تقليل المديونية (دائن)
    INSERT INTO public.journal_entry_lines (
        journal_entry_id, account_id, description, debit_amount, credit_amount, line_number
    ) VALUES (
        journal_entry_id, receivable_account, 'تخفيض مديونية - ' || invoice_number, 0, payment_amount, 3
    );
    
    -- 4. الإيراد المحقق (دائن)
    INSERT INTO public.journal_entry_lines (
        journal_entry_id, account_id, description, debit_amount, credit_amount, line_number
    ) VALUES (
        journal_entry_id, revenue_account, 'إيراد محقق - ' || invoice_number, 0, payment_amount, 4
    );
