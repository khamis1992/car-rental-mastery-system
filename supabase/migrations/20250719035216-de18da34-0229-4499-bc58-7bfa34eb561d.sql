...
CREATE TABLE IF NOT EXISTS public.financial_report_data (
    ...
    comparison_data JSONB DEFAULT '{}', -- للمقارنات مع فترات سابقة
    ...
    file_path TEXT, -- مسار ملف PDF أو Excel
    file_size BIGINT,
    checksum TEXT, -- للتحقق من سلامة البيانات
    created_by UUID,
    notes TEXT
);
...
CREATE TABLE IF NOT EXISTS public.kpi_history (
    ...
    factors JSONB DEFAULT '{}', -- العوامل المؤثرة على المؤشر
    notes TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.financial_forecasts (
    ...
    current_value NUMERIC, -- القيمة الفعلية عند توفرها
    accuracy_percentage NUMERIC, -- دقة التنبؤ
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
    created_by UUID
);
...
CREATE TABLE IF NOT EXISTS public.accounting_dashboard_settings (
    ...
    refresh_interval INTEGER DEFAULT 300, -- بالثواني
...
BEGIN
    FOR tenant_record IN (SELECT id FROM public.tenants WHERE status = 'active') LOOP
        -- قالب تقرير المركز المالي
...
-- إدراج المؤشرات المالية الأساسية
DO $$
DECLARE
    tenant_record RECORD;
BEGIN
    FOR tenant_record IN (SELECT id FROM public.tenants WHERE status = 'active') LOOP
        ...
        INSERT INTO public.advanced_kpis (
            tenant_id, kpi_code, kpi_name, kpi_name_en, kpi_category,
            calculation_formula, target_value, unit_of_measure
        ) VALUES 
        (tenant_record.id, 'CURRENT_RATIO', 'نسبة التداول', 'Current Ratio', 'liquidity', 
         'current_assets / current_liabilities', 2.0, 'ratio'),
        (tenant_record.id, 'QUICK_RATIO', 'النسبة السريعة', 'Quick Ratio', 'liquidity',
         '(current_assets - inventory) / current_liabilities', 1.5, 'ratio'),
        (tenant_record.id, 'CASH_RATIO', 'النسبة النقدية', 'Cash Ratio', 'liquidity',
         'cash_and_equivalents / current_liabilities', 0.5, 'ratio'),
         
        -- مؤشرات الربحية
        (tenant_record.id, 'GROSS_MARGIN', 'هامش الربح الإجمالي', 'Gross Profit Margin', 'profitability',
         '(revenue - cost_of_sales) / revenue * 100', 40.0, 'percentage'),
        (tenant_record.id, 'NET_MARGIN', 'هامش الربح الصافي', 'Net Profit Margin', 'profitability',
         'net_profit / revenue * 100', 15.0, 'percentage'),
        (tenant_record.id, 'ROA', 'العائد على الأصول', 'Return on Assets', 'profitability',
         'net_profit / total_assets * 100', 10.0, 'percentage'),
        (tenant_record.id, 'ROE', 'العائد على حقوق الملكية', 'Return on Equity', 'profitability',
         'net_profit / total_equity * 100', 15.0, 'percentage'),
         
        -- مؤشرات الكفاءة
        (tenant_record.id, 'ASSET_TURNOVER', 'معدل دوران الأصول', 'Asset Turnover', 'efficiency',
         'revenue / average_total_assets', 1.5, 'times'),
        (tenant_record.id, 'RECEIVABLES_TURNOVER', 'معدل دوران المدينين', 'Receivables Turnover', 'efficiency',
         'revenue / average_accounts_receivable', 12.0, 'times'),
         
        -- مؤشرات الرافعة المالية
        (tenant_record.id, 'DEBT_RATIO', 'نسبة الدين', 'Debt Ratio', 'leverage',
         'total_liabilities / total_assets * 100', 50.0, 'percentage'),
        (tenant_record.id, 'EQUITY_RATIO', 'نسبة حقوق الملكية', 'Equity Ratio', 'leverage',
         'total_equity / total_assets * 100', 50.0, 'percentage');
    END LOOP;
END $$;