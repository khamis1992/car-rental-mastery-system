-- Fix hard_delete_tenant function to handle journal entry validation trigger
-- and include comprehensive table deletion
...
BEGIN
    ...
    -- Temporarily disable the journal entry validation trigger
    ALTER TABLE public.journal_entries DISABLE TRIGGER journal_entry_validation_trigger;
    
    -- Delete data in correct order to avoid foreign key violations
    
    -- Level 1: Delete journal entry validations first (causing the constraint issue)
    DELETE FROM public.journal_entry_validations WHERE tenant_id = tenant_id_param;
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('journal_entry_validations: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    
    -- Level 2: Delete child records that reference main tables
    ...
    -- Level 3: Delete main transactional tables
    ...
    -- Journal entries (now safe to delete)
    ...
    -- Level 5: Configuration and settings tables
    ...
    -- Level 6: User and access tables
    ...
    -- Level 7: Finally delete the tenant itself
    ...
    -- Re-enable the journal entry validation trigger
    ALTER TABLE public.journal_entries ENABLE TRIGGER journal_entry_validation_trigger;
