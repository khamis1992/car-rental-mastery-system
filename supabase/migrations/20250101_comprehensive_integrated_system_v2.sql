-- النظام المتكامل الشامل - الإصدار الثاني
-- تم إصلاح مشكلة role_id
-- تاريخ الإنشاء: 2025-01-01 (محدث)

-- ==========================================
-- الجداول الأساسية - مرتبة حسب التسلسل المنطقي
-- ==========================================

-- 1. جدول ملفات المستخدمين (بدون مراجع خارجية)
...
-- 4. جدول ربط المستخدمين بالأدوار (يعتمد على roles)
...
-- إضافة المراجع الخارجية بعد إنشاء جميع الجداول
DO $$ 
BEGIN
    -- إضافة مرجع role_id إلى roles فقط إذا لم يكن موجوداً
    IF NOT EXISTS (
...
('20250101_v2', 'النظام المتكامل الشامل - الإصدار الثاني')
...
DO $$ 
BEGIN
    RAISE NOTICE 'تم إنشاء النظام المتكامل الشامل بنجاح! الإصدار الثاني - تاريخ: %', NOW();
END $$; 