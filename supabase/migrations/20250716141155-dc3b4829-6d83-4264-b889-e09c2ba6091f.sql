-- إكمال دليل الحسابات للبشائر مع جميع الحسابات المطلوبة
DO $$
DECLARE
    tenant_uuid UUID := 'af3acfcf-5c79-43ce-9a92-2e7f1edc25a6'::uuid;
    parent_id UUID;
    accounts_added INTEGER := 0;
BEGIN
    RAISE NOTICE 'بدء إضافة الحسابات المفقودة لمؤسسة البشائر...';
    ...
    (tenant_uuid, '32', 'الأرباح المحتجزة', 'Retained Earnings', 'equity', 'retained_earnings', parent_id, 2, false, true, 0, 0, true),
    (tenant_uuid, '33', 'أرباح السنة الحالية', 'Current Year Earnings', 'equity', 'current_earnings', parent_id, 2, false, true, 0, 0, true)
    ON CONFLICT (tenant_id, account_code) DO NOTHING;
    ...
    INSERT INTO public.chart_of_accounts (tenant_id, account_code, account_name, account_name_en, account_type, account_category, parent_account_id, level, allow_posting, is_active, opening_balance, current_balance, ksaap_compliant)
    VALUES 
    (tenant_uuid, '3201', 'أرباح مرحلة من سنوات سابقة', 'Retained Earnings - Prior Years', 'equity', 'retained_earnings', parent_id, 3, true, true, 0, 0, true),
    (tenant_uuid, '3202', 'احتياطي قانوني', 'Legal Reserve', 'equity', 'retained_earnings', parent_id, 3, true, true, 0, 0, true),
    (tenant_uuid, '3203', 'احتياطي اختياري', 'Optional Reserve', 'equity', 'retained_earnings', parent_id, 3, true, true, 0, 0, true)
    ON CONFLICT (tenant_id, account_code) DO NOTHING;
    ...
    INSERT INTO public.chart_of_accounts (tenant_id, account_code, account_name, account_name_en, account_type, account_category, parent_account_id, level, allow_posting, is_active, opening_balance, current_balance, ksaap_compliant)
    VALUES 
    (tenant_uuid, '51', 'تكلفة البضاعة المباعة', 'Cost of Goods Sold', 'expense', 'cost_of_sales', parent_id, 2, false, true, 0, 0, true),
    ...
    INSERT INTO public.chart_of_accounts (tenant_id, account_code, account_name, account_name_en, account_type, account_category, parent_account_id, level, allow_posting, is_active, opening_balance, current_balance, ksaap_compliant)
    VALUES 
    (tenant_uuid, '5101', 'تكلفة المواد المباشرة', 'Direct Materials Cost', 'expense', 'cost_of_sales', parent_id, 3, true, true, 0, 0, true),
    (tenant_uuid, '5102', 'تكلفة العمالة المباشرة', 'Direct Labor Cost', 'expense', 'cost_of_sales', parent_id, 3, true, true, 0, 0, true),
    (tenant_uuid, '5103', 'مصروفات صناعية غير مباشرة', 'Manufacturing Overhead', 'expense', 'cost_of_sales', parent_id, 3, true, true, 0, 0, true)
    ON CONFLICT (tenant_id, account_code) DO NOTHING;
