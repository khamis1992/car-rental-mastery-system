-- Create comprehensive hard delete tenant function
...
BEGIN
    ...
    INSERT INTO public.tenant_deletion_log (
        ...
        deletion_reason, deletion_type, deletion_date
    ) VALUES (
    ...
    -- Level 1: Detail/child tables first
    ...
    DELETE FROM public.expense_voucher_items WHERE expense_voucher_id IN (
        SELECT id FROM public.expense_vouchers WHERE tenant_id = tenant_id_param
    );
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('expense_voucher_items: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    
    DELETE FROM public.payment_voucher_items WHERE payment_voucher_id IN (
        SELECT id FROM public.payment_vouchers WHERE tenant_id = tenant_id_param
    );
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('payment_voucher_items: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    
    DELETE FROM public.receipt_voucher_items WHERE receipt_voucher_id IN (
        SELECT id FROM public.receipt_vouchers WHERE tenant_id = tenant_id_param
    );
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('receipt_voucher_items: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    
    DELETE FROM public.cost_center_allocations WHERE cost_center_id IN (
        SELECT id FROM public.cost_centers WHERE tenant_id = tenant_id_param
    );
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('cost_center_allocations: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    
    DELETE FROM public.cost_center_budget_alerts WHERE tenant_id = tenant_id_param;
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('cost_center_budget_alerts: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    
    DELETE FROM public.cost_center_history WHERE cost_center_id IN (
        SELECT id FROM public.cost_centers WHERE tenant_id = tenant_id_param
    );
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('cost_center_history: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    ...
    DELETE FROM public.expense_vouchers WHERE tenant_id = tenant_id_param;
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('expense_vouchers: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    
    DELETE FROM public.payment_vouchers WHERE tenant_id = tenant_id_param;
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('payment_vouchers: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    
    DELETE FROM public.receipt_vouchers WHERE tenant_id = tenant_id_param;
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('receipt_vouchers: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    ...
    DELETE FROM public.financial_kpis WHERE tenant_id = tenant_id_param;
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('financial_kpis: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
    
    DELETE FROM public.financial_reports WHERE tenant_id = tenant_id_param;
    GET DIAGNOSTICS records_count = ROW_COUNT;
    IF records_count > 0 THEN
        deleted_tables := deleted_tables || ('financial_reports: ' || records_count::text);
        total_records_deleted := total_records_deleted + records_count;
    END IF;
