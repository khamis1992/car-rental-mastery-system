-- تنفيذ خطة فصل admin@admin.com عن المؤسسات المحددة (مصحح)
...
BEGIN
    ...
    RAISE NOTICE 'Found admin@admin.com user with ID: %', admin_user_id;
    ...
    LOOP
        RAISE NOTICE '- Tenant: %, Role: %, Status: %', tenant_info.name, tenant_info.role, tenant_info.status;
    END LOOP;
