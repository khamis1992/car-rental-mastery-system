-- تصحيح دالة إيراد الدفعة لتوازن القيد المحاسبي
...
BEGIN
    ...
    -- إنشاء القيد المحاسبي (تسجيل الإيراد والتحصيل عند الدفع)
    ...
    INSERT INTO public.journal_entries (
        ...
        journal_entry_number || '-REV',
        payment_date::DATE,
        'إيراد محقق - ' || invoice_number || ' - ' || customer_name,
        'payment_revenue',
        payment_id,
        payment_amount,
        payment_amount,
    ...
    INSERT INTO public.journal_entry_lines (
        ...
        journal_entry_id, revenue_account, 'إيراد تأجير محقق - ' || invoice_number || ' - ' || customer_name, 0, payment_amount, 1
    );
    
    -- سطر مقابل للإيراد (نستخدم حساب الإيرادات المؤجلة إذا كان متاحاً أو المديونية)
    INSERT INTO public.journal_entry_lines (
        journal_entry_id, account_id, description, debit_amount, credit_amount, line_number
    ) VALUES (
        journal_entry_id, receivable_account, 'مقابل الإيراد المحقق - ' || invoice_number || ' - ' || customer_name, payment_amount, 0, 2
    );
