-- إكمال دليل الحسابات لمؤسسة البشائر مع التصنيفات الصحيحة
DO $$
DECLARE
...
BEGIN
    ...
    (bashaer_tenant_id, '52', 'المصروفات الإدارية', 'Administrative Expenses', 'expense', 'operating_expense', parent_id, 2, false, true, 0, 0, true),
    ...
    IF parent_id IS NOT NULL THEN
        INSERT INTO public.chart_of_accounts (tenant_id, account_code, account_name, account_name_en, account_type, account_category, parent_account_id, level, allow_posting, is_active, opening_balance, current_balance, ksaap_compliant)
        VALUES 
        (bashaer_tenant_id, '5201', 'مصروفات إدارية عامة', 'General Administrative Expenses', 'expense', 'operating_expense', parent_id, 3, true, true, 0, 0, true),
        (bashaer_tenant_id, '5202', 'مصروفات الاتصالات', 'Communication Expenses', 'expense', 'operating_expense', parent_id, 3, true, true, 0, 0, true),
        (bashaer_tenant_id, '5203', 'مصروفات القرطاسية والمطبوعات', 'Stationery and Printing', 'expense', 'operating_expense', parent_id, 3, true, true, 0, 0, true),
        (bashaer_tenant_id, '5204', 'مصروفات التأمين', 'Insurance Expenses', 'expense', 'operating_expense', parent_id, 3, true, true, 0, 0, true)
        ON CONFLICT (account_code) DO NOTHING;
    END IF;
