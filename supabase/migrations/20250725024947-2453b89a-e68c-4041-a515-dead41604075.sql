-- إصلاح دالة توليد رقم العميل لتجنب التباس أسماء الأعمدة
...
BEGIN
    ...
    SELECT COALESCE(
        MAX(
            CASE 
                WHEN c.customer_number ~ '^CUS[0-9]+$' 
                THEN substring(c.customer_number FROM 4)::INTEGER
                ELSE 0
            END
        ), 0
    ) INTO last_number
    ...
    -- توليد الرقم الجديد
    new_customer_number := 'CUS' || LPAD((last_number + 1)::TEXT, 6, '0');
