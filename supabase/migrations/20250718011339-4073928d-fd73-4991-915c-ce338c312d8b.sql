-- تحديث إضافي لدالة hard_delete_tenant لإزالة المراجع للجداول غير الموجودة
CREATE OR REPLACE FUNCTION public.hard_delete_tenant(tenant_id_param uuid, deletion_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    -- Level 1: Delete child records that reference main tables
    ...
    -- Contract related child tables (only existing ones)
    ...
    -- Customer-related tables (only existing ones)
    ...
    -- Vehicle-related tables (check if they exist before deleting)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'vehicle_costs' AND table_schema = 'public') THEN
    ...
    -- Skip maintenance_logs as it doesn't exist
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'vehicle_documents' AND table_schema = 'public') THEN
    ...
    -- Level 2: Delete main transactional tables
    ...
    -- Check if suppliers table exists before deleting
    ...
    -- Check if fixed_assets table exists before deleting
    ...
    -- Check if cost_centers table exists before deleting
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'cost_centers' AND table_schema = 'public') THEN
    ...
    -- Check if departments table exists before deleting
    ...
    -- Check if office_locations table exists before deleting
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'office_locations' AND table_schema = 'public') THEN
