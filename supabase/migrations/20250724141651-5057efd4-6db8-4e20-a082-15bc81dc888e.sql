...
-- إضافة SET search_path TO 'public' للدوال مع استخدام المعاملات الصحيحة

-- إصلاح الدوال الموجودة بنفس المعاملات
...
BEGIN
    ...
    (target_tenant_id, '3', 'حقوق الملكية', 'Equity', 'equity', 'capital', 1, false, true, 0, 0),
    ...
    SELECT id INTO parent_id FROM public.chart_of_accounts WHERE tenant_id = target_tenant_id AND account_code = '1';
    ...
    VALUES 
    (target_tenant_id, '11', 'الأصول المتداولة', 'Current Assets', 'asset', 'current_asset', parent_id, 2, false, true, 0, 0),
    (target_tenant_id, '12', 'الأصول الثابتة', 'Fixed Assets', 'asset', 'fixed_asset', parent_id, 2, false, true, 0, 0);
...
BEGIN
    ...
    (target_tenant_id, 'CC001', 'مركز التكلفة الرئيسي', 'department', 1, 'CC001', true, 0, 0),
    (target_tenant_id, 'CC002', 'مركز تكلفة السيارات', 'vehicle', 2, 'CC001.CC002', true, 0, 0);
...
BEGIN
    ...
    INSERT INTO public.company_branding (
        ...
        target_tenant_id, 'اسم الشركة', 'Company Name',
        '#007bff', '#6c757d'
    );
END;
$function$;
