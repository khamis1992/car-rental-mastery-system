-- إنشاء جدول المعاملات البنكية المستوردة
CREATE TABLE public.bank_reconciliation_imports (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    bank_account_id UUID NOT NULL REFERENCES public.bank_accounts(id),
    import_date DATE NOT NULL DEFAULT CURRENT_DATE,
    file_name TEXT,
    file_size INTEGER,
    total_transactions INTEGER DEFAULT 0,
    matched_transactions INTEGER DEFAULT 0,
    unmatched_transactions INTEGER DEFAULT 0,
    import_status TEXT NOT NULL DEFAULT 'pending' CHECK (import_status IN ('pending', 'processing', 'completed', 'failed')),
    imported_by UUID REFERENCES auth.users(id),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id),
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    notes TEXT
);

-- إنشاء جدول المعاملات البنكية المستوردة
CREATE TABLE public.imported_bank_transactions (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    import_id UUID NOT NULL REFERENCES public.bank_reconciliation_imports(id) ON DELETE CASCADE,
    bank_account_id UUID NOT NULL REFERENCES public.bank_accounts(id),
    transaction_date DATE NOT NULL,
    description TEXT NOT NULL,
    reference_number TEXT,
    debit_amount NUMERIC(15,3) DEFAULT 0,
    credit_amount NUMERIC(15,3) DEFAULT 0,
    balance_after NUMERIC(15,3),
    bank_reference TEXT,
    check_number TEXT,
    matched_journal_entry_id UUID REFERENCES public.journal_entries(id),
    match_confidence NUMERIC(3,2) DEFAULT 0, -- 0.00 to 1.00
    match_type TEXT CHECK (match_type IN ('manual', 'automatic', 'suggested')),
    matched_at TIMESTAMP WITH TIME ZONE,
    matched_by UUID REFERENCES auth.users(id),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id),
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    is_matched BOOLEAN DEFAULT false,
    match_notes TEXT
);

-- إنشاء جدول مطابقة المعاملات
CREATE TABLE public.bank_reconciliation_matches (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    imported_transaction_id UUID NOT NULL REFERENCES public.imported_bank_transactions(id),
    journal_entry_id UUID NOT NULL REFERENCES public.journal_entries(id),
    match_amount NUMERIC(15,3) NOT NULL,
    match_confidence NUMERIC(3,2) DEFAULT 1.00,
    match_type TEXT NOT NULL DEFAULT 'manual' CHECK (match_type IN ('manual', 'automatic', 'suggested')),
    match_reason TEXT,
    matched_by UUID NOT NULL REFERENCES auth.users(id),
    matched_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    tenant_id UUID NOT NULL REFERENCES public.tenants(id),
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    is_confirmed BOOLEAN DEFAULT false,
    notes TEXT
);

-- إنشاء جدول تقارير المطابقة البنكية
CREATE TABLE public.bank_reconciliation_reports (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    bank_account_id UUID NOT NULL REFERENCES public.bank_accounts(id),
    reconciliation_date DATE NOT NULL,
    opening_balance NUMERIC(15,3) NOT NULL DEFAULT 0,
    closing_balance NUMERIC(15,3) NOT NULL DEFAULT 0,
    book_balance NUMERIC(15,3) NOT NULL DEFAULT 0,
    total_deposits NUMERIC(15,3) NOT NULL DEFAULT 0,
    total_withdrawals NUMERIC(15,3) NOT NULL DEFAULT 0,
    outstanding_deposits NUMERIC(15,3) NOT NULL DEFAULT 0,
    outstanding_withdrawals NUMERIC(15,3) NOT NULL DEFAULT 0,
    bank_charges NUMERIC(15,3) NOT NULL DEFAULT 0,
    interest_earned NUMERIC(15,3) NOT NULL DEFAULT 0,
    reconciled_balance NUMERIC(15,3) NOT NULL DEFAULT 0,
    variance_amount NUMERIC(15,3) NOT NULL DEFAULT 0,
    reconciliation_status TEXT NOT NULL DEFAULT 'draft' CHECK (reconciliation_status IN ('draft', 'in_progress', 'completed', 'approved')),
    prepared_by UUID NOT NULL REFERENCES auth.users(id),
    approved_by UUID REFERENCES auth.users(id),
    approved_at TIMESTAMP WITH TIME ZONE,
    tenant_id UUID NOT NULL REFERENCES public.tenants(id),
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    notes TEXT
);

-- Enable RLS
ALTER TABLE public.bank_reconciliation_imports ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.imported_bank_transactions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.bank_reconciliation_matches ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.bank_reconciliation_reports ENABLE ROW LEVEL SECURITY;

-- RLS Policies for bank_reconciliation_imports
CREATE POLICY "المحاسبون والمديرون يمكنهم إدارة استيراد المعاملات البنكية" 
ON public.bank_reconciliation_imports 
FOR ALL 
USING (has_role(auth.uid(), 'admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role) OR has_role(auth.uid(), 'accountant'::user_role))
WITH CHECK (has_role(auth.uid(), 'admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role) OR has_role(auth.uid(), 'accountant'::user_role));

-- RLS Policies for imported_bank_transactions
CREATE POLICY "المحاسبون والمديرون يمكنهم إدارة المعاملات البنكية المستوردة" 
ON public.imported_bank_transactions 
FOR ALL 
USING (has_role(auth.uid(), 'admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role) OR has_role(auth.uid(), 'accountant'::user_role))
WITH CHECK (has_role(auth.uid(), 'admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role) OR has_role(auth.uid(), 'accountant'::user_role));

-- RLS Policies for bank_reconciliation_matches
CREATE POLICY "المحاسبون والمديرون يمكنهم إدارة مطابقة المعاملات البنكية" 
ON public.bank_reconciliation_matches 
FOR ALL 
USING (has_role(auth.uid(), 'admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role) OR has_role(auth.uid(), 'accountant'::user_role))
WITH CHECK (has_role(auth.uid(), 'admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role) OR has_role(auth.uid(), 'accountant'::user_role));

-- RLS Policies for bank_reconciliation_reports
CREATE POLICY "المحاسبون والمديرون يمكنهم إدارة تقارير المطابقة البنكية" 
ON public.bank_reconciliation_reports 
FOR ALL 
USING (has_role(auth.uid(), 'admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role) OR has_role(auth.uid(), 'accountant'::user_role))
WITH CHECK (has_role(auth.uid(), 'admin'::user_role) OR has_role(auth.uid(), 'manager'::user_role) OR has_role(auth.uid(), 'accountant'::user_role));

-- إنشاء فهارس لتحسين الأداء
CREATE INDEX idx_bank_reconciliation_imports_bank_account ON public.bank_reconciliation_imports(bank_account_id);
CREATE INDEX idx_bank_reconciliation_imports_tenant ON public.bank_reconciliation_imports(tenant_id);
CREATE INDEX idx_imported_bank_transactions_import ON public.imported_bank_transactions(import_id);
CREATE INDEX idx_imported_bank_transactions_bank_account ON public.imported_bank_transactions(bank_account_id);
CREATE INDEX idx_imported_bank_transactions_matched ON public.imported_bank_transactions(is_matched);
CREATE INDEX idx_bank_reconciliation_matches_transaction ON public.bank_reconciliation_matches(imported_transaction_id);
CREATE INDEX idx_bank_reconciliation_matches_journal ON public.bank_reconciliation_matches(journal_entry_id);
CREATE INDEX idx_bank_reconciliation_reports_bank_account ON public.bank_reconciliation_reports(bank_account_id);
CREATE INDEX idx_bank_reconciliation_reports_date ON public.bank_reconciliation_reports(reconciliation_date);